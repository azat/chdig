...
impl ClickHouse {
    ...
    pub async fn get_slow_query_log(
        ...
        let (id_condition, ids_cte) = if self
            .quirks
            .has(ClickHouseAvailableQuirks::AdditionalTableFiltersInSubquery)
        {
            let ids_cte = format!(
                r#",
                        slow_queries_ids AS (
            ...
            (
                "initial_query_id GLOBAL IN slow_queries_ids".to_string(),
                ids_cte,
            )
        } else {
            ...
            (
                format!("initial_query_id IN ('{}')", ids.join("','")),
                "".to_string(),
            )
        };
    ...
    pub async fn get_last_query_log(
            ...
            .has(ClickHouseAvailableQuirks::AdditionalTableFiltersInSubquery)
        {
            ...
            (
                "initial_query_id GLOBAL IN last_queries_ids".to_string(),
                ids_cte,
            )
        } else {
            let ids_columns = self
    ...
    pub async fn kill_queries(&self, query_ids: &[String]) -> Result<()> {
        let ids = quote_sql_strings(query_ids);
        let query = if let Some(cluster) = &self.opts().cluster {
            format!(
                "KILL QUERY ON CLUSTER {} WHERE query_id IN ({}) SYNC",
                cluster, ids
            )
        } else {
            format!("KILL QUERY WHERE query_id IN ({}) SYNC", ids)
        };
