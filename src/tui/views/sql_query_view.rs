...
// No Debug derive: the framework's StyledString does not implement Debug.
...
type RowCallback = Arc<dyn Fn(&mut App, Vec<&'static str>, Row) + Send + Sync>;
...
impl SQLQueryView {
    ...
    where
        F: Fn(&mut App, Vec<&'static str>, Row) + Send + Sync + 'static,
    {
...
impl Component for SQLQueryView {
