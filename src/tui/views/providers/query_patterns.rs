use super::query_patterns_metrics::{self, METRICS, Metric};
use crate::{
...
fn open_last_queries_for_hash(app: &mut App, columns: Vec<&'static str>, row: QueryResultRow) {
...
fn apply_metric_change(app: &mut App, context: ContextArc) {
...
fn show_metric_picker(app: &mut App) {
    let items: Vec<(String, String)> = METRICS
        .iter()
        .map(|m| (m.label.to_string(), m.key.to_string()))
        .collect();
    fuzzy_select_strings(app, "Select metric", items, |app, key| {
...
fn build_and_install(app: &mut App, context: ContextArc) {
    let metric = context.lock().unwrap().query_patterns_metric;
    ...
    view.get_inner_mut().set_color_log_scale(
        "p90",
        vec![Color::Green, Color::Yellow, Color::Magenta, Color::Red],
    );
    ...
    app.present_view(VIEW_NAME, wrapped);
}
