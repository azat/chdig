...
impl ProcessesView {
    ...
    fn show_flamegraph(&mut self, tui: bool, trace_type: Option<TraceType>) -> Result<()> {
        ...
        if let Some(trace_type) = trace_type {
            context_locked
                .worker
                .send_force(WorkerEvent::ShowQueryFlameGraph(
                    trace_type,
                    tui,
            ...
            context_locked
                .worker
                .send_force(WorkerEvent::ShowLiveQueryFlameGraph(tui, Some(query_ids)));
        }
    ...
    pub fn new(
        ...
        context.add_view_action(
            ...
            |v| {
                ...
                context_locked
                    .worker
                    .send_force(WorkerEvent::ExecuteQuery(database, query));
        ...
        context.add_view_action(&mut event_view, "EXPLAIN SYNTAX", 's', |v| {
            ...
            context_locked
                .worker
                .send_force(WorkerEvent::ExplainSyntax(database, query, settings));
        ...
        context.add_view_action(&mut event_view, "EXPLAIN PLAN", 'e', |v| {
            ...
            context_locked
                .worker
                .send_force(WorkerEvent::ExplainPlan(database, query));
        ...
        context.add_view_action(&mut event_view, "EXPLAIN PIPELINE", 'E', |v| {
            ...
            context_locked
                .worker
                .send_force(WorkerEvent::ExplainPipeline(database, query));
        ...
        context.add_view_action(
            ...
            |v| {
                ...
                context_locked
                    .worker
                    .send_force(WorkerEvent::ExplainPipelineOpenGraphInBrowser(
                        database, query,
                    ));
        ...
        context.add_view_action(&mut event_view, "EXPLAIN INDEXES", 'I', |v| {
            ...
            context_locked
                .worker
                .send_force(WorkerEvent::ExplainPlanIndexes(database, query));
                        ...
                        views::Dialog::new()
                            ...
                            .button("Yes, I'm sure", move |s| {
                                context_copy
                                    ...
                                    .unwrap()
                                    .worker
                                    .send_force(WorkerEvent::KillQuery(query_id.clone()));
